import { ACTIONS, TOTAL, shuffle, generateRound, checkAnswer, getResultText, type ActionRound } from './logic';
import { playToddlerCorrect, playToddlerWrong, playWin, initMuteButton } from '../../lib/sounds';
import { getHighScore, setHighScore, setLastPlayed } from '../../lib/storage';
import { showConfetti } from '../../lib/confetti';
import { trackGameStart, trackGameEnd, trackRating, createRatingUI } from '../../lib/analytics';

const GAME_ID = 'toddler-actions';
let _analyticsStartTime = 0;
let queue: typeof ACTIONS;
let current: ActionRound;
let score = 0, round = 0;
...
function nextRound() {
  ...
  const target = queue[round % queue.length];
  current = generateRound(target, ACTIONS);
  ...
  container.innerHTML = '';
  current.choices.forEach(action => {
    const btn = document.createElement('button');
    btn.className = 'choice-btn';
    btn.innerHTML = `<span class="action-emoji">${action.emoji}</span><span class="action-name">${action.name}</span>`;
    btn.onclick = () => pick(btn, action.name);
...
function pick(btn: HTMLButtonElement, answer: string) {
  ...
  if (correct) {
    ...
    document.querySelectorAll<HTMLButtonElement>('.choice-btn').forEach(b => {
      if (b.querySelector('.action-name')?.textContent === current.target.name) b.classList.add('correct');
    });
  }
...
if (best > 0) {
  ...
  el.style.cssText = 'color:rgba(255,255,255,0.4);font-size:0.9rem;margin-top:0.5rem;';
