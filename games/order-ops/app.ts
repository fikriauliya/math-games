import { generateQuestion, checkAnswer, calcScore, getGrade, type Question } from './logic';
import { playCorrect, playWrong, playWin, playCombo, initMuteButton } from '../../lib/sounds';
import { getHighScore, setHighScore, setLastPlayed } from '../../lib/storage';
import { showConfetti } from '../../lib/confetti';
import { trackGameStart, trackGameEnd, createRatingUI } from '../../lib/analytics';

const GAME_ID = 'order-ops';
let currentQ: Question;
let score = 0, correct = 0, wrong = 0, streak = 0, bestStreak = 0, qIdx = 0, total = 15;
let diff = 'medium';
let startTime = 0;
...
function startGame() {
  ...
  score = correct = wrong = streak = bestStreak = qIdx = 0;
  startTime = Date.now();
...
function nextQ() {
  ...
  document.getElementById('expression')!.textContent = currentQ.expression;
...
function answer(val: number, btn: HTMLButtonElement) {
  ...
  if (checkAnswer(val, currentQ.answer)) {
    ...
    score += calcScore(streak);
    streak >= 3 ? playCombo(streak) : playCorrect();
  } else {
    btn.classList.add('wrong');
    document.querySelectorAll<HTMLButtonElement>('.choice-btn').forEach(b => { if (parseInt(b.textContent!) === currentQ.answer) b.classList.add('correct'); });
    wrong++; streak = 0;
    playWrong();
  }
  qIdx++;
