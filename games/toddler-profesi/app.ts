import { PROFESI_LIST, TOTAL, shuffle, genRound, checkAnswer, getResult, type Profesi } from './logic';
import { playToddlerCorrect, playToddlerWrong, playWin, initMuteButton } from '../../lib/sounds';
import { getHighScore, setHighScore, setLastPlayed } from '../../lib/storage';
import { showConfetti } from '../../lib/confetti';
import { trackGameStart, trackGameEnd, createRatingUI } from '../../lib/analytics';

const GAME_ID = 'toddler-profesi';
let _start = 0, queue: Profesi[], current: Profesi, score = 0, round = 0;
...
function startGame() {
  ...
  queue = shuffle([...PROFESI_LIST]).slice(0, TOTAL);
  score = 0; round = 0;
...
function nextRound() {
  ...
  const choices = genRound(current, PROFESI_LIST);
