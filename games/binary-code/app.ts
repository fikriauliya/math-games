...
function nextRound() {
  ...
  current = genQuestion(round); updateUI();
...
function endGame() {
  ...
  if (isNew && score > 0) { const el = document.createElement('div'); el.textContent = '🎉 NEW RECORD!'; el.style.cssText = 'font-size:1.5rem;font-weight:900;color:#00ff00;animation:pulse 0.5s infinite alternate;margin:0.5rem 0;'; document.getElementById('result-title')!.after(el); }
  if (score === TOTAL) showConfetti();
}

const best = getHighScore(GAME_ID);
if (best > 0) { const el = document.createElement('div'); el.textContent = `🏆 Best: ${best}/${TOTAL}`; el.style.cssText = 'color:#00ff00;font-size:0.9rem;margin-top:0.5rem;'; document.querySelector('.btn-play')?.before(el); }
initMuteButton();
