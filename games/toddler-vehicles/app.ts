import { VEHICLES, TOTAL, shuffle, getOptions, checkAnswer, getResultText, type Vehicle } from './logic';
import { playToddlerCorrect, playToddlerWrong, playWin, initMuteButton } from '../../lib/sounds';
import { getHighScore, setHighScore, setLastPlayed } from '../../lib/storage';
import { showConfetti } from '../../lib/confetti';
import { trackGameStart, trackGameEnd, trackRating, createRatingUI } from '../../lib/analytics';

const GAME_ID = 'toddler-vehicles';
let _analyticsStartTime = 0;
let queue: Vehicle[], current: Vehicle, score: number, round: number;
...
function startGame() {
  ...
  queue = shuffle([...VEHICLES]).slice(0, TOTAL);
  score = 0; round = 0;
  show('game-screen');
  renderStars();
  nextRound();
...
function nextRound() {
  ...
  const options = getOptions(current, VEHICLES);
  const container = document.getElementById('choices')!;
  container.innerHTML = '';
  options.forEach(opt => {
    const btn = document.createElement('button');
    btn.className = 'choice-btn';
    btn.innerHTML = `<span class="vehicle-emoji">${opt.emoji}</span>`;
