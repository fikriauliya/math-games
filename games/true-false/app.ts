import { generateStatement, checkAnswer, getResultText, GAME_DURATION, type TFStatement } from './logic';
...
let score: number, timeLeft: number, timer: any, current: TFStatement;
...
function nextStatement() {
  ...
  requestAnimationFrame(() => card.style.animation = 'slideIn 0.3s ease');
...
function pick(answer: boolean) {
  ...
  document.querySelectorAll<HTMLButtonElement>('.tf-btn').forEach(b => {
    b.disabled = true;
    const isCorrectBtn = (b.dataset.answer === 'true') === current.answer;
    if (isCorrectBtn) b.classList.add('correct');
    if ((b.dataset.answer === 'true') === answer && !correct) b.classList.add('wrong');
  });

  updateHUD();
  setTimeout(nextStatement, 350);
}
