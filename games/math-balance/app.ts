...
function nextQ() {
  ...
  leftEl.innerHTML = q.leftSide.map(n => `<span class="weight">${n}</span>`).join('+');
  rightEl.innerHTML = `<span class="weight mystery">?</span>+<span class="weight">${q.rightTarget}</span>`;
...
function answer(val: number, btn: HTMLButtonElement) {
  ...
  if (ok) { btn.classList.add('correct'); state.correct++; state.score += 10; playCorrect(); document.querySelector('.mystery')!.textContent = String(val); }
