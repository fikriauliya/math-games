import { FOODS, TOTAL, shuffle, generateRound, checkAnswer, getResultText, type FoodRound } from './logic';
import { playToddlerCorrect, playToddlerWrong, playWin, initMuteButton } from '../../lib/sounds';
import { getHighScore, setHighScore, setLastPlayed } from '../../lib/storage';
import { showConfetti } from '../../lib/confetti';
import { trackGameStart, trackGameEnd, trackRating, createRatingUI } from '../../lib/analytics';

const GAME_ID = 'toddler-food';
let _analyticsStartTime = 0;
let queue: typeof FOODS;
let current: FoodRound;
let score = 0, round = 0;
...
function nextRound() {
  ...
  current = generateRound(target, FOODS);

  speak(`Mana ${current.target.name}?`);
  document.getElementById('question-text')!.textContent = `Mana ${current.target.name}?`;

  const container = document.getElementById('choices')!;
  container.innerHTML = '';
  current.choices.forEach(food => {
    const btn = document.createElement('button');
    btn.className = 'choice-btn';
    btn.innerHTML = `<span class="food-emoji">${food.emoji}</span><span class="food-name">${food.name}</span>`;
    btn.onclick = () => pick(btn, food.name);
...
function pick(btn: HTMLButtonElement, answer: string) {
  ...
  if (correct) {
    ...
    document.querySelectorAll<HTMLButtonElement>('.choice-btn').forEach(b => {
      if (b.querySelector('.food-name')?.textContent === current.target.name) b.classList.add('correct');
    });
  }
