import { WEATHERS, TOTAL, shuffle, generateQuestion, checkAnswer, getResultText, type Weather, type WeatherQuestion } from './logic';
import { playToddlerCorrect, playToddlerWrong, playWin, initMuteButton } from '../../lib/sounds';
import { getHighScore, setHighScore, setLastPlayed } from '../../lib/storage';
import { showConfetti } from '../../lib/confetti';
import { trackGameStart, trackGameEnd, trackRating, createRatingUI } from '../../lib/analytics';

const GAME_ID = 'toddler-weather';
let _analyticsStartTime = 0;
let queue: Weather[];
let current: WeatherQuestion;
let score = 0, round = 0;
...
function nextRound() {
  ...
  const remaining = queue.slice(round);
  current = generateQuestion([], remaining);
  
  const qText = document.getElementById('question-text')!;
  qText.textContent = `Mana ${current.target.name.toUpperCase()}?`;
  speak(`Mana ${current.target.name}?`);
  ...
  current.options.forEach(opt => {
    ...
    btn.innerHTML = `<span class="weather-emoji">${opt.emoji}</span><span class="weather-name">${opt.name}</span>`;
    btn.onclick = () => pick(btn, opt.name);
...
function pick(btn: HTMLButtonElement, answer: string) {
  ...
  if (correct) {
    ...
    renderStars();
    playToddlerCorrect();
    speak(`Benar! Ini ${current.target.name}!`);
  } else {
    playToddlerWrong();
    document.querySelectorAll<HTMLButtonElement>('.choice-btn').forEach(b => {
      if (b.querySelector('.weather-name')!.textContent === current.target.name) b.classList.add('correct');
    });
    speak(`Ini ${current.target.name}`);
  }
...
if (best > 0) {
  ...
  el.style.cssText = 'color:rgba(21,101,192,0.6);font-size:0.9rem;margin-top:0.5rem;';
