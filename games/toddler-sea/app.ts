import { ANIMALS, TOTAL, shuffle, generateRound, checkAnswer, getResultText, type SeaAnimal, type RoundData } from './logic';
import { playToddlerCorrect, playToddlerWrong, playWin, initMuteButton } from '../../lib/sounds';
import { getHighScore, setHighScore, setLastPlayed } from '../../lib/storage';
import { showConfetti } from '../../lib/confetti';
import { trackGameStart, trackGameEnd, createRatingUI } from '../../lib/analytics';

const GAME_ID = 'toddler-sea';
...
function nextRound() {
  ...
  container.innerHTML = '';
  currentRound.choices.forEach(animal => {
    const btn = document.createElement('button');
    btn.className = 'animal-btn';
    btn.innerHTML = `<span class="animal-emoji">${animal.emoji}</span><span class="animal-name">${animal.name}</span>`;
    btn.onclick = () => pick(btn, animal);
    container.appendChild(btn);
  });
}

function pick(btn: HTMLButtonElement, animal: SeaAnimal) {
  document.querySelectorAll<HTMLButtonElement>('.animal-btn').forEach(b => b.onclick = null);
  const correct = checkAnswer(animal.name, currentRound.target.name);
  if (correct) {
    ...
    playToddlerWrong();
    document.querySelectorAll<HTMLButtonElement>('.animal-btn').forEach(b => {
      if (b.querySelector('.animal-name')!.textContent === currentRound.target.name) b.classList.add('correct');
    });
