...
function startGame() {
  ...
  state = { score: 0, correct: 0, wrong: 0, qIdx: 0,
...
function nextQ() {
  ...
  q.choices.forEach(c => {
    ...
    btn.className = 'choice-btn'; btn.textContent = `X = ${c}`;
...
function answer(val: number, btn: HTMLButtonElement) {
  ...
  else { btn.classList.add('wrong'); state.wrong++; playWrong(); document.querySelectorAll<HTMLButtonElement>('.choice-btn').forEach(b => { if (parseInt(b.textContent!.replace('X = ','')) === state.currentQ!.answer) b.classList.add('correct'); }); }
